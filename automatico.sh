...
project="Floresta-Tropical"			    #Desbloqueado no Automatico.sh para conter o nome do projeto do Github
...
function criar_automatico()
{
	...
	if [ $enter = "Y" -o $enter = "y" ]; then
		...
		sed "s/project="Floresta-Tropical"/project=\"$project\"/g" "temp1.sh" > "temp2.sh"
		echo "-    |--------------------%50                    |      -"
...
function executar()
{
	...
	clear
	cabeca
	echo "- Não  é  feito  nenhuma  configuração,  portando  para -"
	echo "-   para executar é precisa estar previamente funcional -"
	echo "-                                                       -" 
	echo "- 1.Push                                                -"
	echo "- 2.Pull                                                -"
	echo "- 0.Sair                                                -"
...
if [ $autoself -eq 1 ]; then
	...
	if [ -e "automatico.sh" ]; then
		clear
		cabeca
		echo "- O automatico.sh  do  projeto  já  existe.  Você  deve -"
		echo "-    utilizar o automatico.sh                           -"
